...
function toFormState(order: Record<string, unknown>) {
  return {
    ...
    estimatedDeliveryAt: (order.estimatedDeliveryAt as string)
...
export default component$(() => {
  ...
  const loadOrders = $(async () => {
    ...
    try {
      ...
      const openList = Array.isArray(openData)
        ? (openData as Record<string, unknown>[])
        : [];
      const cancelledList = Array.isArray(cancelledData)
  ...
  const activeList =
    activeTab.value === "active" ? openOrders : cancelledOrders;
    ...
    <section class="admin-card rounded-xl p-4 shadow-sm sm:p-6">
      ...
      <div class="mt-5 grid gap-3 md:grid-cols-[1fr_200px]">
        ...
        <select
          value={sortBy.value}
          onChange$={(_, el) =>
            (sortBy.value = (el as HTMLSelectElement).value)
          }
          class="rounded-2xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 outline-none transition focus:border-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
        >
      ...
      <div class="mt-6 space-y-3">
        {loading.value ? (
          ...
          filtered.map((order) => {
            const allowed =
              activeTab.value === "active"
                ? getAllowedActions(order.status as string)
                : [];
            const user = order.user as Record<string, unknown> | undefined;
            const items = (order.items as Record<string, unknown>[] | []) || [];
            const form =
              orderForms[order.id as string | number] || toFormState(order);
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                      {order.createdAt
                        ? formatMediumDate(order.createdAt as string)
                        : ""}{" "}
                      · {(order.status as string) ?? "—"}
                    </p>
                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">
                      {user?.name ? `${user.name as string}` : "Guest"}{" "}
                ...
                <div class="mt-3 space-y-2">
                  {items.map((item: Record<string, unknown>, idx: number) => {
                    ...
                    return (
                      <div
                        key={idx}
                        class="flex items-center gap-3 rounded-lg border border-slate-100 p-2 dark:border-slate-800"
                      >
                        {product?.image ? (
                          <img
                          ...
                          <div class="flex h-10 w-10 items-center justify-center rounded bg-slate-100 text-xs text-slate-400 dark:bg-slate-800">
                            No img
                          </div>
                        )}
                        <div class="flex-1">
                          <p class="text-sm font-medium">
                            {(product?.name as string | undefined) ?? "Item"}
                          </p>
                  ...
                  <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-200 pt-3 dark:border-slate-800">
                    ...
                    {(order.status as string) !== "cancelled" && (
                      <button
                        ...
                        onClick$={() =>
                          handleRefund(order.id as string | number)
                        }
                      >
                        Refund
                      </button>
