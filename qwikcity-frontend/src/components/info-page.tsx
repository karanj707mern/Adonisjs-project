...
export const InfoPage = component$<{ pageKey: string }>(({ pageKey }) => {
  const page = PAGE_CONTENT[pageKey] ?? PAGE_CONTENT["about-us"];

  return (
    <div class="min-h-screen bg-[var(--bg-primary)] px-4 py-12 text-[var(--text-primary)] sm:px-6 sm:py-16 lg:px-10">
      <div class="mx-auto max-w-7xl">
        <Link href="/" class="btn-secondary inline-flex">
          Back to home
        </Link>

        <div class="mt-8 rounded-[2rem] border border-[var(--border-color)] bg-[var(--bg-secondary)] p-6 shadow-sm sm:mt-10 sm:p-10">
          <p class="text-sm uppercase tracking-[0.16em] text-emerald-700 dark:text-emerald-300">
            {page.eyebrow}
          </p>
          <h1 class="mt-4 font-serif text-3xl leading-tight text-[var(--text-primary)] sm:text-4xl lg:text-5xl">
            {page.title}
          </h1>
          <p class="mt-6 max-w-3xl text-base leading-8 text-[var(--text-secondary)]">
            {page.description}
          </p>
          ...
          <div class="mt-10 space-y-6">
            {page.sections.map((section) => (
              <section
                key={section.heading}
                class="rounded-[1.5rem] bg-[var(--bg-primary)] p-5 sm:p-6"
              >
