...
export function useCurrentUser(): CurrentUserStore {
  const store = useStore<CurrentUserStore>({ user: getCurrentUser() });
...
export interface AuthCheckedStore {
  checked: boolean;
}

export function useAuthChecked(): AuthCheckedStore {
  const store = useStore<AuthCheckedStore>({ checked: getAuthChecked() });

  useVisibleTask$(() => {
    const sync = () => {
      store.checked = getAuthChecked();
    };

    sync();
    window.addEventListener(AUTH_CHECKED_EVENT, sync);

    return () => {
      window.removeEventListener(AUTH_CHECKED_EVENT, sync);
    };
  });
