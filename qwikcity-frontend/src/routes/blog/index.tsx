...
export const useBlogPosts = routeLoader$(async () => {
  try {
    ...
    return { posts: [], error: err instanceof Error ? err.message : "Could not load posts." };
  }
});
...
export default component$(() => {
  ...
  return (
    <div class="container-page py-10">
      ...
      <p class="mt-2 text-slate-500">Tips, recipes and stories about living well with moringa.</p>
      ...
      {data.value.error ? (
        ...
        <div class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {data.value.posts.map((post) => (
            <a key={post.id} href={`/blog/${post.slug ?? post.id}`} class="card group overflow-hidden">
              <div class="aspect-video overflow-hidden bg-slate-100 dark:bg-slate-800">
                {post.image ? (
                  <img src={resolveImageUrl(post.image)} alt={post.title} loading="lazy" class="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
                ) : null}
              </div>
              <div class="p-4">
                <h2 class="font-semibold leading-tight group-hover:text-neon">{post.title}</h2>
                {post.excerpt ? <p class="mt-2 line-clamp-3 text-sm text-slate-500 dark:text-slate-400">{post.excerpt}</p> : null}
                {post.publishedAt || post.createdAt ? (
                  <p class="mt-3 text-xs text-slate-400">{formatMediumDate(post.publishedAt ?? post.createdAt)}</p>
                ) : null}
              </div>
            </a>
          ))}
