...
function buildMissingActivities(
  ...
  maybeAdd(OrderStatus.PENDING, order.createdAt, 'Order received and queued for processing.')
  maybeAdd(OrderStatus.PAID, order.paidAt, 'Payment was confirmed for this order.')
  maybeAdd(OrderStatus.SHIPPED, order.shippedAt, 'The order was shipped.')
  maybeAdd(OrderStatus.OUT_FOR_DELIVERY, order.outForDeliveryAt, 'The order is out for delivery.')
  maybeAdd(OrderStatus.DELIVERED, order.deliveredAt, 'The order was delivered successfully.')
  ...
  if (order.status === OrderStatus.CANCELLED && !seenStatuses.has(OrderStatus.CANCELLED)) {
    activityCreates.push({
      ...
      createdAt: order.activities[order.activities.length - 1]?.createdAt ?? order.createdAt,
    })
  }
...
async function main() {
  ...
  const patches = orders
    ...
    .filter(({ patch }) => Object.keys(patch.data).length > 0 || patch.missingActivities.length > 0)

  const orderUpdateCount = patches.filter(({ patch }) => Object.keys(patch.data).length > 0).length
  ...
  if (!shouldApply) {
    console.log('[legacy-orders] No changes written. Re-run with --apply to update the database.')
    return
  }
