...
function buildMissingActivities(
  ...
  maybeAdd(
    OrderStatus.PENDING,
    order.createdAt.toJSDate(),
    'Order received and queued for processing.',
  );
  maybeAdd(
    OrderStatus.PAID,
    order.paidAt?.toJSDate() ?? null,
    'Payment was confirmed for this order.',
  );
  maybeAdd(
    OrderStatus.SHIPPED,
    order.shippedAt?.toJSDate() ?? null,
  ...
  maybeAdd(
    ...
    order.deliveredAt?.toJSDate() ?? null,
  ...
  if (
    ...
    activityCreates.push({
      ...
      createdAt:
        order.activities[order.activities.length - 1]?.createdAt.toJSDate() ??
        order.createdAt.toJSDate(),
    });
  }
