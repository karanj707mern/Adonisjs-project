...
import CartService from './cart_service.ts'
import {
  createCartValidator,
  updateCartValidator,
  mergeGuestCartValidator,
  guestCartValidator,
} from './cart_validators.ts'
...
export default class CartController {
  ...
  async create(ctx: HttpContext) {
    ...
    return this.cartService.create(userId, data.productId, data.quantity ?? 1, guestToken)
  }
  ...
  async createGuestCart(ctx: HttpContext) {
    ...
    const result = await this.cartService.createGuestCart(data.items ?? [], guestToken)
  ...
  async mergeGuestCart(ctx: HttpContext) {
    ...
    if (!token) {
      throw { status: 400, message: 'Guest cart token is required' }
    }
