...
export default function registerCart(router) {
  router.group(() => {
    router.post('cart', [CartController, 'create']);
      ...
      router.get('cart/guest', [CartController, 'getGuestCart']).as('cart.get_guest_cart');
      router.get('cart/guest/:token', [CartController, 'getGuestCart']).as('cart.get_guest_cart_by_token');
      router.post('cart/guest/merge', [CartController, 'mergeGuestCart']);
      router.delete('cart/guest', [CartController, 'deleteGuestCart']).as('cart.delete_guest_cart');
      router.delete('cart/guest/:token', [CartController, 'deleteGuestCart']).as('cart.delete_guest_cart_by_token');
  }).prefix('Cart');
}
