...
import AuthService, { allowedImageMimeTypes } from './auth_service.ts'
import AuthCookiesService from './services/auth_cookies_service.ts'
import DeviceInfoService from './services/device_info_service.ts'
import CaptchaService from './services/captcha_service.ts'
...
export default class AuthController {
  ...
  async login({ request, response }: HttpContext) {
    ...
    if (!dto.captchaId || !dto.captchaInput) {
      throw { status: 400, message: 'CAPTCHA verification is required' }
    }

    const isValid = await this.captchaService.verifyCaptcha(dto.captchaId, dto.captchaInput)
    if (!isValid) {
    ...
    this.cookiesService.setAuthCookies(response, result.accessToken, result.refreshToken)
  ...
  async googleAuth({ request, response }: HttpContext) {
    ...
    const deviceInfo = this.deviceInfoService.extractDeviceInfo(request)
    const guestToken = (request as any).guestToken
    const result = await this.authService.googleAuth({ credential: dto.credential }, deviceInfo)
  ...
  async refresh({ request, response }: HttpContext) {
    ...
    response.cookie('accessToken', result.accessToken, this.getAuthCookieOptions(60 * 60 * 1000))
    response.cookie(
      'refreshToken',
      result.refreshToken,
  ...
  async session({ request, response }: HttpContext) {
    try {
      ...
      const refreshToken = request.cookie('refreshToken')
      const result = await this.authService.getSession(accessToken, refreshToken)

      if (result.authenticated && result.refreshed && result.accessToken && result.refreshToken) {
        this.cookiesService.setAuthCookies(response, result.accessToken, result.refreshToken)
      }
  ...
  async resetPassword({ request }: HttpContext) {
    ...
    return this.authService.resetPassword({ token: dto.token, password: dto.password })
  }
  ...
  async uploadAvatar({ auth, request }: HttpContext) {
    ...
    if (!file) {
      throw { status: 400, message: 'An avatar image is required.' }
    }

    const mimeType = file.type ?? ''
    if (!(mimeType in allowedImageMimeTypes)) {
      throw { status: 400, message: 'Only JPG, PNG, WEBP, AVIF, and GIF images are allowed.' }
    }
    ...
    const fileName = `${Date.now()}-${file.clientName ?? 'avatar'}`
    ...
    const result = await this.storage.uploadFile(
      { buffer, mimetype: mimeType, originalname: file.clientName ?? fileName },
      'avatars'
    )
