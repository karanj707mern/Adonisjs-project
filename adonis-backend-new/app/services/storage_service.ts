...
export default class StorageService {
  ...
  constructor() {
    ...
    if (this.cloudName && this.apiKey && this.apiSecret) {
      ...
      logger.warn('Cloudinary credentials missing; uploads fall back to local storage')
    }
  }
  ...
  async uploadFile(
    ...
    if (this.isCloudinaryProvider) {
      try {
        ...
        logger.warn(`Cloudinary upload failed, falling back to local: ${(error as Error).message}`)
      }
    }
  ...
  async deleteFile(key: string): Promise<void> {
    ...
    try {
      ...
      logger.warn(`Cloudinary delete failed for ${key}: ${(error as Error).message}`)
    }
  ...
  private async validateImage(buffer: Buffer): Promise<void> {
    try {
      ...
      if (!metadata.format) throw { status: 400, message: 'Invalid image file' }
    } catch {
      throw { status: 400, message: 'Invalid image file' }
    }
  }
  ...
  private sanitize(value: string): string {
    ...
    const safe = parts.filter((part) => part !== '..').join('/')
